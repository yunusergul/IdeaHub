...
async function main() {
  ...
  // --- Bulk Votes & Comments for bulk ideas ---
  const commentTemplates = [
    'Bu konuda daha detaylı bir analiz yapılmalı.',
    'Harika bir öneri, kesinlikle destekliyorum!',
    'Bence önce pilot bir çalışma yapılmalı.',
    'Bu projede ben de yer almak isterim.',
    'Maliyet analizi yapıldı mı? Bütçe etkisini bilmek isterim.',
    'Benzer bir çalışmayı geçen çeyrekte yapmıştık, deneyimlerimizi paylaşabilirim.',
    'Zaman çizelgesi nasıl olacak? Tahmini bir süre var mı?',
    'Diğer ekiplerle koordinasyon gerekecek, erken başlamalıyız.',
    'Teknik olarak uygulanabilir ama operasyonel zorluklar olabilir.',
    'Müşteri geri bildirimleri de bu yönde, önceliklendirmeliyiz.',
    'Alternatif çözümler de değerlendirilmeli bence.',
    'Risk analizi yapmadan ilerlemek riskli olabilir.',
    'Proof of concept ile başlayalım derim.',
    'Bu konu hakkında bir workshop düzenlesek çok faydalı olur.',
    'Tam olarak ihtiyacımız olan şey, +1!',
  ];

  const replyTemplates = [
    'Katılıyorum, iyi bir yaklaşım olur.',
    'Ben de aynı şeyi düşünüyordum.',
    'Bu konuda bir toplantı ayarlayalım mı?',
    'Detayları Slack\'te konuşalım.',
    'Güzel bir bakış açısı, teşekkürler.',
  ];

  for (let idx = 0; idx < bulkIdeaTitles.length; idx++) {
    const ideaId = `bulk-i-${idx + 1}`;

    // Random votes (0-6 voters)
    const voterCount = Math.floor(Math.random() * 7);
    const shuffledVoters = [...users].sort(() => Math.random() - 0.5).slice(0, voterCount);
    for (const voter of shuffledVoters) {
      await prisma.vote.upsert({
        where: { ideaId_userId: { ideaId, userId: voter.id } },
        update: {},
        create: { ideaId, userId: voter.id, type: 'up' },
      });
    }
    // Update upvotes count to match actual votes
    await prisma.idea.update({
      where: { id: ideaId },
      data: { upvotes: voterCount },
    });

    // Random comments (0-4 root comments, some with 0-2 replies)
    const rootCommentCount = Math.floor(Math.random() * 5);
    let totalComments = 0;
    for (let ci = 0; ci < rootCommentCount; ci++) {
      const commentId = `${ideaId}-c${ci + 1}`;
      ...
      for (let ri = 0; ri < replyCount; ri++) {
        const replyId = `${commentId}-r${ri + 1}`;
    ...
    // Update commentCount to match actual comments
    await prisma.idea.update({
      where: { id: ideaId },
  ...
  console.log('Created bulk votes and comments');
